...
class StreamHandlerVideocamera(socketserver.StreamRequestHandler):
    ...
    def handle(self):
        stream_bytes = b' '
        global ultrasonic_sensor_distance
        global comando_cliente
        # Object detection initialization (STOP sign, traffic light) using cascade classifiers
        self.obj_detection = ObjectDetection()
        # Imprime el directorio actual para efectos de desarrollo
        curr_directory = os.getcwd()
        print("Directorio actual: {}".format(curr_directory))
        self.stop_cascade = cv2.CascadeClassifier('scripts/cascade_xml/stop_sign.xml')
        self.light_cascade = cv2.CascadeClassifier('scripts/cascade_xml/traffic_light.xml')
...
# Clase para manejo de mensajes para control de motores
class StreamHandlerMotores(socketserver.BaseRequestHandler):
    
    def handle(self):
        cliente = self.request
        # Envía el comando al cliente
        cliente.send(bytes(comando_cliente.encode("utf-8")))
...
class ThreadServer( object ):
    ...
    # Hilo del servidor para manejo de los motores
    def server_thread_motores(host, port):
        print('+ Iniciando transmisión de control de movimiento ' + str(host) + ':' + str(port))
        server = socketserver.TCPServer((host, port), StreamHandlerMotores)
        server.serve_forever()
    ...
    thread_motores = threading.Thread( name= 'thread_motores', target=server_thread_motores, args=(server_ip, server_port_mensaje))
